...
def test_network():
    ...
    sum_op1 = operation(name='sum_op1', needs=['a', 'b'], provides='sum_ab')(
        add
    )
    ...
    @operation(
        name='mul_op1', needs=['sum_ab', 'b'], provides='sum_ab_times_b'
    )
    def mul_op1(a, b):
        return a * b
    ...
    # compose network
    net = compose(name='my network')(
        sum_op1, mul_op1, pow_op1, sum_op2, sum_op3
    )
...
def test_network_simple_merge():
    ...
    sum_op3 = operation(name='sum_op3', needs=['sum1', 'c'], provides='sum3')(
        add
    )
...
def test_network_deep_merge():
    ...
    sum_op5 = operation(name='sum_op4', needs=['sum1', 'b'], provides='sum2')(
        add
    )
    net2 = compose(name='my network 2')(sum_op4, sum_op5)
    pprint(net2({'a': 1, 'b': 2}))
...
def test_input_based_pruning():
    ...
    sum_op2 = operation(name='sum_op2', needs=['a', 'b'], provides='sum2')(add)
    sum_op3 = operation(
        name='sum_op3', needs=['sum1', 'sum2'], provides='sum3'
    )(add)
...
def test_output_based_pruning():
    ...
    sum_op3 = operation(name='sum_op3', needs=['c', 'sum2'], provides='sum3')(
        add
    )
    net = compose(name='test_net')(sum_op1, sum_op2, sum_op3)
...
def test_deleted_optional():
    ...
    sum_op2 = operation(
        name='sum_op2', needs=['sum1', 'sum1'], provides='sum2'
    )(add)
...
def test_backwards_compatibility():
    ...
    # get specific outputs
    pprint(
        net.compute(outputs=['sum_ab_times_b'], named_inputs={'a': 1, 'b': 2})
    )

    # start with inputs already computed
    pprint(
        net.compute(
            outputs=['sum_ab_times_b'], named_inputs={'sum_ab': 1, 'b': 2}
        )
    )
